...
function getErrorFilePath(error) {
  ...
  const filePath = filePathLine
    .trim()
    .replace(/^at (?:new )?Object\.<anonymous> \((.*)\)$/, "$1");
  return filePath;
}
...
async function handleUnhandledRejection() {
  process.on("uncaughtException", (err) => {
    ...
    console.error(
      chalk.white("[ ") +
        chalk.hex("#ec0303")("ERROR") +
        chalk.white(" ]") +
        chalk.greenBright(` Caught exception: ${chalk.yellow(err.message)}`)
    );
    console.error(
      chalk.white("[ ") +
        chalk.hex("#ec0303")("ORIGIN ERROR") +
        chalk.white(" ]") +
        chalk.greenBright(` ${chalk.yellow(filePath)}`)
    );
  });
}
